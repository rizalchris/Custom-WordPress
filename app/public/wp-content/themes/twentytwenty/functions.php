...
function enqueue_scripts()
{
	...
	wp_enqueue_script('crud-js', get_template_directory_uri() . '/js/crud.js', array('jquery'), null, true);
	wp_enqueue_script('load-career', get_template_directory_uri() . '/js/load-career.js', array('jquery'), null, true);

	// Localize script
	wp_localize_script('crud-js', 'ajax_obj', array(
...
function handle_crud_operations()
{
	check_ajax_referer('crud_nonce', 'nonce');

	global $wpdb;
	$table_name = $wpdb->prefix . 'product_category';
	...
	if ($action === 'create' || $action === 'update') {
		$data = array(
			'product_category_name' => sanitize_text_field($_POST['product_category_name']),
			'product_category_slug' => sanitize_text_field($_POST['product_category_slug']),
			'product_category_desc' => sanitize_text_field($_POST['product_category_desc']),
			'product_category_status' => intval($_POST['product_category_status']),
		...
		if ($action === 'create') {
			...
			$wpdb->update($table_name, $data, array('product_category_id' => $product_category_id));
			$response = array('status' => 'success', 'message' => 'Category updated successfully');
		}
	} elseif ($action === 'delete') {
		$product_category_id = intval($_POST['product_category_id']);
		$wpdb->delete($table_name, array('product_category_id' => $product_category_id));
		...
		$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name LIMIT %d, %d", $start, $length));
...
add_action('wp_ajax_crud_operations', 'handle_crud_operations');
add_action('wp_ajax_nopriv_crud_operations', 'handle_crud_operations');

//Career
function load_career_content()
{
